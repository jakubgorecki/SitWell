...
public class TimerModel{
    ...
    public void extendTime(){
        restart();
        if((double)timeToLong()*1.1 > 90*60 || (double)timeToLong()== 90*60)
            saveTime(90*60);
        else
            saveTime((long)((double)timeToLong()*1.1));
    }
    
    public void shortenTime(){
        restart();
        if((double)timeToLong()*0.9 < 15*60 || (double)timeToLong()== 15*60)
            saveTime(15*60);
        else
            saveTime((long)((double)timeToLong()*0.9));
    }
    ...
    private String loadTimeValue(){
        ...
        try{
            BufferedReader fileReader = new BufferedReader( new FileReader("dynamic.txt"));
